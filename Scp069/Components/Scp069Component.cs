...
namespace Scp069.Components
{
    public class Scp069Component : MonoBehaviour
    {
        ...
        private void Start()
        {
            InvokeRepeating(nameof(DoDamage), 0.1f, 1f);
        ...
        public void RegisteringEvents()
        {
            ...
            Log.Debug($"{scp069.Nickname} Became SCP-069 Initializing component.", Plugin.Instance.Config.ShowDebug);
        }
        public void UnRegisteringEvents()
        {
            ...
            Log.Debug($"Destroying {this} which was {(scp069 == null ? nully : scp069.Nickname)}", Plugin.Instance.Config.ShowDebug);
        }
        ...
        private void OnKill(DyingEventArgs ev)
        {
            ...
            ev.Killer.ChangeAppearance(scp069roletype);
            //Movement speed
        ...
        {
            scp069.ChangeAppearance(scp069roletype);
        }
        ...
        private void UpdateItemOnHand(ItemType it)
        {
            try
            {
                ...
                {
                    player.SendFakeSyncVar(scp069.ReferenceHub.networkIdentity, typeof(Inventory), nameof(Inventory.Network_curItemSynced), (sbyte)it);
                }
            }
            catch (Exception e)
            {
                Log.Error($"Error while updating held item: {e}");
            }
        }
    }
}
